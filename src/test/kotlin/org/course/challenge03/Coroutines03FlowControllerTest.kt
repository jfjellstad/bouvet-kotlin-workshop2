...
class Coroutines03FlowControllerTest @Autowired constructor(val stockRepository: StockRepository,
    ...
    fun `Exercise A should get stock by id`(): Unit = runBlocking {
        webTestClient.get().uri("/stocks").exchange().apply {
            expectStatus().isOk
            expectBody<List<Stock>>().returnResult().responseBody shouldBe stockRepository.findAll().toList()
        }
    }
    ...
    fun `Exercise B should consume newly added Stocks via ServerSentEvents (SSE) emitted from the pollingStockFlow endpoint stocks sse-polling`(): Unit = runBlocking {
        ...
        retryTillOk { consumed shouldBe stockRepository.findAll().toList() }

        //now we add a new stock to the repository
        val newStock = stockRepository.save(Stock(symbol = NEWS, price = 2345.12))
